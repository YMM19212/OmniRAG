...
首页用于汇总系统连接状态、集合统计信息和最近访问记录，方便快速确认当前工作台是否已经完成初始化并处于可用状态。

![OmniRAG 首页](./docs_images/homepage.png)

### 配置

配置页用于初始化知识库、切换 Milvus Lite 与独立 Milvus，并调整 embedding、集合名和去重等运行参数。

![OmniRAG 配置页](./docs_images/config.png)

### 导入

导入页支持文本、图片、视频的单条导入与批量导入，并提供跳过重复、提取缩略图等选项。

![OmniRAG 导入页](./docs_images/input.png)

### 检索

检索页支持文本、图片、视频查询，并提供模态过滤、相似度阈值和结果查看能力，适合执行语义检索与结果比对。

![OmniRAG 检索页](./docs_images/retrieval.png)
